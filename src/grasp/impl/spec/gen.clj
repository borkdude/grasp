...
(ns grasp.impl.spec.gen
    (:refer-clojure :exclude [boolean bytes cat hash-map list map not-empty set vector
                              char double int keyword symbol string uuid delay shuffle])
    #_(:require [clojure.test.check]
              [clojure.test.check.generators]
              [clojure.test.check.properties]))

(alias 'c 'clojure.core)

(def ^:private quick-check-ref (c/delay #_clojure.test.check/quick-check))
(defn quick-check
  [& args]
  (apply @quick-check-ref args))

(def ^:private for-all*-ref
     (c/delay #_(clojure.test.check.properties/for-all*)))
(defn for-all*
  "Dynamically loaded clojure.test.check.properties/for-all*."
  [& args]
  (apply @for-all*-ref args))

(let [g? identity #_clojure.test.check.generators/generator?
      g identity #_clojure.test.check.generators/generate
      mkg identity #_clojure.test.check.generators/->Generator]
  (defn- generator?
    [x]
    (g? x))
  (defn- generator
